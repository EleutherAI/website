...
- (Looking at standardized templates) The best hyperparameter settings (all the ones involving variance and no negation consistency; 0.648) don't significantly outperform the difference in means reporter ($w\_{sup}=1$, everthing else 0 $\rightarrow$ 0.63 transfer AUROC) or the CRC reporter ($w\_{cov}=w\_{var}=0.5$ and everything else 0 $\rightarrow$ 0.611 transfer AUROC).
