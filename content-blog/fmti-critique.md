...
Central to the transparency of LLMs is whether or not the weights of the model in question are available. Without the weights, many of the claims made in a model report are unverifiable beyond the word of the organization which owns it. Open release of model weights is covered by "Full external model access" in the Index, which means that it represents exactly one point out of one hundred in the Index. In a previous version we did not acknowledge this, and we regret the oversight.

We do not think that a single point adequately represents the importance of released weights to genuine transparency. Distinctions between evaluations based upon models with open weights, which are reproducible and can be verified by other researchers, are not made by the Index when evaluating model basics, capabilities, limitations, risks, mitigations, or trustworthiness. Organizations that do not release model weights are not being transparent about those things, and should not be scored as being transparent about them. Failing to make a distinction between verifiable and unverifiable claims represents the clearest failure in understanding how open-source and academic communities engage within the LLM space, where transparency is a primary goal, but one that is not rewarded significantly by the FMTI.

In communicating with the authors of the Index, they state that releasing model weights is rewarded by the FMTI, and that developers that make the weights of their models fully available score points on Full External Access, External Model Access Protocol, Black Box Eternal Model Access, Versioning Protocol, Change Log, and Deprecation Policy. Upon review of the FMTI scoring, it can be seen that all of these points except Full External Model Access are awarded to OpenAI, which has not released any model weights for GPT-4. Full External Model Access is the only point that reflects the availability of model weights.
