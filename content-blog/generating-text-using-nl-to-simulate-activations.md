...
- We aim to find a lower bound on what fraction of latents need to be recovered for near-SAE-g.t. performance
- We find 50% of most active latents or 90% of random active latents give a good approximation of SAE g.t.
- With current techniques, we found that this level of recall is difficult to achieve without swamping the signal with false positives, see section "Correctly identifying non-active latents"
...
- We aim to find what is the recall of current interpretations to identify active latents, and what factors influence it.
- We find that that the model is able to identify 50% of the most active latents, and that the recall is higher for more active latents.
