#!/usr/bin/env python3
"""
SUPERSEDED (v4). The post's live AI-2027 figure (ai2027-smooth-vs-jump.png) and
the other current figures are generated by the v5 scripts in
_scratch/review/verify-v5/. This file's embedded model is the OLD v4 (ℓ/ℓ_k
bounded-gate) engine, and its outputs (ai2027-high-leakage-run.png,
ai2027-observability-cannot-save.png) are no longer referenced by the post. The
canonical engine is now the v5 src/BasinExplorer.jsx (two-stage b→s monitoring,
single O_fix gate, ρ fix rate, γ turnover). Kept for reference only; do not
regenerate post figures from here.
